...
class EditSaleModalContent extends StatefulWidget {
  static final GlobalKey<_EditSaleModalContentState> globalKey = GlobalKey();
  int saleId;
  String saleCode;
  String saleDate;
  String saleDeadline;
  ...
  EditSaleModalContent(
      ...
      required this.saleDeadline,
      required this.saleDesc})
      : super(key: globalKey);

  @override
  State<EditSaleModalContent> createState() => _EditSaleModalContentState();
}

class _EditSaleModalContentState extends State<EditSaleModalContent> {
  //List
  List<Order> orders = [];
  ...
  late Future orderSaleFuture;
  getOrdersGroup() async {
    print("TOL");
    var response = await _orderService.getOrdersGrouped();
    response.data.forEach((order) {
      if (order.saleId == widget.saleId) {
        print("ADA NIH : ${order}");
        selectedOrder = order;

        _orderCode = selectedOrder.orderCode;
        getExtraDiscounts(selectedOrder.orderCode!);
      }
    });
  }
  ...
  createSale() async {
    print("TESTING");
    var response = await _saleService.updateSale(
  ...
  void initState() {
    ...
    super.initState();
    // getOrders();
    orderSaleFuture = getOrdersGroup();

    _saleCodeController.text = widget.saleCode;
    saleDate = DateTime.parse(widget.saleDate);
    saleDeadline = DateTime.parse(widget.saleDeadline);
    _descController.text = widget.saleDesc;
